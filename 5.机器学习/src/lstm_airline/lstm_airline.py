...
def trainData(look_back,errArray):
    ...
    trainX = numpy.reshape(trainX, (samples, 1, trainX.shape[1]))

    samples = testX.shape[0]
    testX = numpy.reshape(testX, (samples, 1, testX.shape[1]))
    ...
    model = Sequential()
    model.add(LSTM(4, input_shape=(1,look_back)))#time_steps,features
    model.add(Dense(1))
    ...
    errArray.append([look_back,trainScore,testScore])
...
csv_write("error.csv", errArray)




plt.plot(mydata['c1'], mydata['c2'], 'r.--', mydata['c1'], mydata['c3'], 'go--')
plt.xlabel('feature_number', size=14)
